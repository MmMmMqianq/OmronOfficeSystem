...
# 1. xlrd常用函数
data = xlrd.open_workbook("demo.xls")  # 返回工作簿对象
# data.sheet_loaded(0)  # 判断是否加载了工作表，可以指定工作表索引或者工作表名字
# data.unload_sheet(1)  # 卸载工作表，可以指定工作表索引或者工作表名字

print("1. ", data.sheets())  # 返回工作簿中的所有工作表列表
print("2. ", data.sheets()[0])  # 通过列表索引访问工作表
print("3. ", data.sheet_by_index(1))  # 通过工作索引访问工作表
print("4. ", data.sheet_by_name("Sheet3"))  # 通过名字访问工作表

print("5. ", data.sheet_names())  # 返回所有工作表的名字
print("6. ", data.nsheets)  # 返回工作表的数量
