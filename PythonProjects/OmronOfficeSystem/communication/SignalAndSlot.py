...
class SignalAndSlot(QWidget):
	def __init__(self):
		...
		self.meta_object3_1 = self.send.signal3[int, str, int].connect(self.slot.slot3_1)
		print("7, self.send.signal3[int, str, int].connect(self.defSignal.slot3_1) = ", self.meta_object3_1)
...
class Signal(QObject):
	...
	signal3 = pyqtSignal([int, str, int], [str, int])  # 信号名字为signal3，带两个重载参数，两个中括号是或的关系
	...
	def connect_and_emit_signal3_1(self):
		self.signal3[int, str, int].emit(10, "十", 15)
...
class Slot(QObject):
	...
	def slot3_1(self, int1, str1, int2):
		print("3. signal3返回的两个数据为：%d, %s, %d" % (int1, str1, int2))
