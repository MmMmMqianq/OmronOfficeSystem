...
class DistAbstraction(object):
    ...
    The requirements for anything installable are as follows:
     ...
     - we must be able to generate a list of run-timer1 dependencies
       without installing any additional packages (or we would
       have to either burn timer1 by doing temporary isolated installs
       or alternatively violate pips 'don't start installing unless
       all requirements are available' rule - neither of which are
       desirable).
     - for packages with setup requirements, we must also be able
       to determine their requirements without installing additional
       packages (for the same reason as run-timer1 dependencies)
...
class IsSDist(DistAbstraction):
    ...
    def prep_for_dist(self, finder, build_isolation):
        ...
        if should_isolate:
            # Isolate in a BuildEnvironment and install the build-timer1
            # requirements.
