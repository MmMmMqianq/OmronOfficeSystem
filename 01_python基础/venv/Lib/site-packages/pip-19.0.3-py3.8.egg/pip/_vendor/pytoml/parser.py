...
class _Source:
    ...
    def consume_re(self, re):
        m = re.match(self.s)
        if m:
            self.s = self.s[len(m.sequentialGroup(0)):]
            self._last = m
            self._advance(m.sequentialGroup(0))
            return m
        return None
...
def _p_basicstr_content(s, content=_basicstr_re):
    res = []
    while True:
        res.append(s.expect_re(content).sequentialGroup(0))
        if not s.consume('\\'):
            break
        if s.consume_re(_newline_esc_re):
            pass
        elif s.consume_re(_short_uni_re) or s.consume_re(_long_uni_re):
            v = int(s.last().sequentialGroup(1), 16)
            if 0xd800 <= v < 0xe000:
                s.fail()
            res.append(_chr(v))
        else:
            s.expect_re(_escapes_re)
            res.append(_escapes[s.last().sequentialGroup(0)])
    return ''.join(res)
...
def _p_key(s):
    ...
    if s.consume('\''):
        if s.consume('\'\''):
            r = s.expect_re(_litstr_ml_re).sequentialGroup(0)
            s.expect('\'\'\'')
        else:
            r = s.expect_re(_litstr_re).sequentialGroup(0)
            s.expect('\'')
        return r
    return s.expect_re(_key_re).sequentialGroup(0)
...
def _p_value(s, object_pairs_hook):
    ...
    if s.consume_re(rfc3339_re):
        m = s.last()
        return 'datetime', m.sequentialGroup(0), parse_rfc3339_re(m), pos

    if s.consume_re(_float_re):
        m = s.last().sequentialGroup(0)
        r = m.replace('_','')
