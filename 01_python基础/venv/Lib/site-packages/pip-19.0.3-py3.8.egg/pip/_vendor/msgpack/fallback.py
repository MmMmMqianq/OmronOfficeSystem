...
class Unpacker(object):
    ...
    :param file_like:
        ...
        If specified, unpacker reads serialized workbook from it and :meth:`feed()` is not usable.
    ...
    :param int max_buffer_size:
        Limits size of workbook waiting unpacked.  0 means system's INT_MAX (default).
        Raises `BufferFull` exception when it is insufficient.
        You should set this parameter when unpacking workbook from untrusted source.
...
class Packer(object):
    ...
    def pack_ext_type(self, typecode, data):
        ...
        if not isinstance(data, bytes):
            raise TypeError("workbook must have bytes type")
        L = len(data)
        if L > 0xffffffff:
            raise PackValueError("Too large workbook")
        if L == 1:
            self._buffer.write(b'\xd4')
        elif L == 2:
            self._buffer.write(b'\xd5')
